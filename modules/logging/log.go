...
func registerLog(b *bot.Bot) {
	...
	b.GroupMemberJoinEvent.Subscribe(func(qqClient *client.QQClient, event *client.MemberJoinGroupEvent) {
		logger.
			WithField("from", "GroupMemberJoin").
			WithField("GroupCode", event.Group.Code).
			WithField("MemberID", event.Member.Uin).
			Info("group member join")
	})
}
